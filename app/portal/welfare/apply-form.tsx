...
import { applyWelfareClaimAsMemberAction } from "@/lib/actions/welfare";
...
function SubmitButton() {
  ...
  return (
    ...
    >
      {pending ? "กำลังส่งคำขอ..." : "ยื่นคำขอสวัสดิการ"}
    </button>
  );
}

export function ApplyWelfareForm({
  funds,
}: {
  funds: { id: string; name: string }[];
}) {
  const [state, formAction] = useActionState<ActionState, FormData>(
    applyWelfareClaimAsMemberAction,
    {}
  );
